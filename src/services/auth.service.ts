...
const signToken = (payload: object, expires: any = '15m') => {
    return jwt.sign(payload, process.env.JWT_SECRET || 'secret', {
        expiresIn: expires
    });
};
...
const buildAuthenticatedSession = async (user: any, ip: string, device: string, auditAction = '2FA Verification Success') => {
    ...
    if (user.role === 'PATIENT' && staffRecords.length === 0) {
        const patientRecords = await prisma.patient.findMany({
            where: { email: user.email },
            include: { clinic: { select: { id: true, name: true, location: true, status: true, currency: true } } }
        });
...
export const login = async (data: any, ip: string, device: string) => {
    ...
    if (user.sessionToken) {
        ...
        const dbTimeResult = await prisma.$queryRawUnsafe<any[]>('SELECT NOW() as dbNow');
        ...
        if (isSessionActive) {
            throw new AppError(
                'This account is already logged in from another device. Please log out from the existing session first.',
                409
            );
        }
    }
    ...
    if (isPatientRole) {
        ...
        const patientRecords = await prisma.patient.findMany({
            where: { email: user.email },
            include: { clinic: { select: { id: true, name: true, location: true, status: true, logo: true, currency: true } } }
        });
...
// ─────────────────────────────────────────────────────────────────────────────
export const logout = async (userId: number, ip: string, device: string) => {
    await prisma.user.update({
        where: { id: userId },
    ...
    return staffRecords
        ...
        .map((record: any) => {
            ...
            return {
                ...
                location: record.clinic.location,
                status: record.clinic.status,
                currency: record.clinic.currency,
                subdomain: record.clinic.subdomain
            };
        });
};
...
export const selectClinic = async (userId: number, clinicId: number, role: string, ip: string, device: string) => {
    ...
    if (isPatient && !isSuperAdmin) {
        const patientRecord = await prisma.patient.findFirst({
            where: { email: user.email, clinicId },
            include: { clinic: { select: { id: true, name: true, status: true, location: true, currency: true } } }
        });
