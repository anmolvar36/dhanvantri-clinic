...
export const requestSuperAdminContact = async (requestedClinicId: number, message: string) => {
    let clinic = await prisma.clinic.findUnique({ where: { id: Number(requestedClinicId) || 0 } });
    if (!clinic) {
        clinic = await prisma.clinic.findFirst();
    }
    if (!clinic) throw new AppError('Clinic not found', 404);

    const safeMessage = message ? message.trim() : 'Request to enable additional modules.';
    const textPayload = `Message from ${clinic.name}: "${safeMessage}"`;

    const notification = await prisma.notification.create({
        data: {
            clinicId: clinic.id,
            department: 'SUPER_ADMIN',
            // Send standard attributes for a polished text string layout without category tags
            message: JSON.stringify({ text: textPayload })
        }
    });
