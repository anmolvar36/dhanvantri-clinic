...
export const getDoctorShortcuts = async (userId: number, clinicId: number) => {
    try {
        const rows = await (prisma as any).doctor_shortcuts.findMany({
...
export const addDoctorShortcut = async (userId: number, clinicId: number, type: string, text: string) => {
    try {
        const existing = await (prisma as any).doctor_shortcuts.findFirst({
            where: { userId, clinicId, type, text }
        });
        if (existing) return existing;

        return await (prisma as any).doctor_shortcuts.create({
            data: { userId, clinicId, type, text }
        });
    } catch (e) {
        return null;
    }
...
export const deleteDoctorShortcut = async (userId: number, clinicId: number, type: string, text: string) => {
    try {
        await (prisma as any).doctor_shortcuts.deleteMany({
            where: { userId, clinicId, type, text }
        });
    } catch (e) {
        // ignore
