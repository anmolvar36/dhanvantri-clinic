...
export const requestSuperAdminContact = asyncHandler(async (req: AuthRequest, res: Response) => {
    const notification = await clinicService.requestSuperAdminContact(req.clinicId!, req.body.message);
    res.status(201).json({ success: true, message: 'Message sent to Super Admin successfully', data: notification });
});
