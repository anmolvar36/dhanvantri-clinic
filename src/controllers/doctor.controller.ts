...
export const getShortcuts = asyncHandler(async (req: AuthRequest, res: Response) => {
    const clinicId = Number(req.clinicId || req.user?.clinicId || 0);
    const shortcuts = await doctorService.getDoctorShortcuts(req.user!.id, clinicId);
    res.status(200).json({ status: 'success', data: shortcuts });
});

export const addShortcut = asyncHandler(async (req: AuthRequest, res: Response) => {
    const clinicId = Number(req.clinicId || req.user?.clinicId || 0);
    const shortcut = await doctorService.addDoctorShortcut(req.user!.id, clinicId, req.body.type, req.body.text);
    res.status(201).json({ status: 'success', data: shortcut });
});

export const deleteShortcut = asyncHandler(async (req: AuthRequest, res: Response) => {
    const clinicId = Number(req.clinicId || req.user?.clinicId || 0);
    await doctorService.deleteDoctorShortcut(req.user!.id, clinicId, String(req.params.type), String(req.params.text));
    res.status(200).json({ status: 'success', message: 'Shortcut removed' });
});
