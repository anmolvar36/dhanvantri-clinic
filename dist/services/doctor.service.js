...
export const getAllClinicPatients = async (clinicId) => {
    return await prisma.patient.findMany({
        where: { clinicId },
        include: {
            medicalrecord: {
...
export const getDoctorShortcuts = async (userId, clinicId) => {
    try {
        const rows = await prisma.doctor_shortcuts.findMany({
...
export const addDoctorShortcut = async (userId, clinicId, type, text) => {
    try {
        const existing = await prisma.doctor_shortcuts.findFirst({
            where: { userId, clinicId, type, text }
        });
        if (existing)
            return existing;
        return await prisma.doctor_shortcuts.create({
...
export const deleteDoctorShortcut = async (userId, clinicId, type, text) => {
    try {
        await prisma.doctor_shortcuts.deleteMany({
            where: { userId, clinicId, type, text }
        });
    }
    catch (e) {
