...
async function migrate() {
    try {
        ...
        const result = await prisma.$queryRawUnsafe(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS 
        ...
        if (result.length > 0) {
            ...
            await prisma.$executeRawUnsafe('ALTER TABLE `user` ADD COLUMN `sessionToken` VARCHAR(191) NULL');
