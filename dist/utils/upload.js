...
const fileFilter = (_req, file, cb) => {
    if (!ALLOWED_MIME_TYPES.includes(file.mimetype)) {
        return cb(new AppError(`Invalid file type "${file.mimetype}". Only JPEG, PNG, and WebP images are allowed.`, 400));
    }
    cb(null, true);
};
    ...
    single: (fieldName) => {
        return (req, res, next) => {
            memUpload.single(fieldName)(req, res, async (err) => {
                ...
                if (!isValidImage) {
                    return next(new AppError('File content does not match the declared image type. Upload rejected.', 400));
                }
